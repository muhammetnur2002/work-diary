...
router.get('/', async (req, res) => {
    ...
    `SELECT * FROM entries WHERE user_id = ? AND strftime('%Y-%m', date) = ? ORDER BY date ASC`
    );
...
router.post('/', async (req, res) => {
    ...
    if (exists) {
        myDb.run(
        `UPDATE entries SET start_time = ?, end_time = ?, comment = ?, hourly_rate = ?, updated_at = datetime('now') WHERE user_id = ? AND date = ?`,
        [start_time || null, end_time || null, comment || '', hourly_rate || null, req.userId, date]
        );
    } else {
        myDb.run(
        `INSERT INTO entries (user_id, date, start_time, end_time, comment, hourly_rate) VALUES (?, ?, ?, ?, ?, ?)`,
...
router.put('/:id', async (req, res) => {
    ...
    myDb.run(
    `UPDATE entries SET start_time = ?, end_time = ?, comment = ?, hourly_rate = ?, updated_at = datetime('now') WHERE id = ?`,
